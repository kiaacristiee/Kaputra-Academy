...
import { BookOpen, Calendar, DollarSign, PlusCircle, CheckCircle2, ChevronRight, AlertCircle, Sparkles, Clock } from "lucide-react";
...
export default function EnrollClient({ initialCourses, initialCamps, studentId }: EnrollClientProps) {
  ...
  const handleCampEnroll = async () => {
    ...
    const res = await enrollInCamp(studentId, selectedCamp.id);
    if (res.success && res.invoiceId) {
      setSuccessType("CAMP");
